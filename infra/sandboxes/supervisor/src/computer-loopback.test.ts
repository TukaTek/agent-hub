...
describe("space computer limit enforcement", () => {
  ...
  async function provisionBot(
    ...
    return supervisorApp.request("/computers", {
      ...
      headers: {
        authorization: `Bearer ${resolveSupervisorToken(process.env)}`,
        "content-type": "application/json",
        "x-cortexai-agent-hub-bot-id": botId,
        "x-cortexai-agent-hub-space-id": spaceId,
      },
  ...
  it("rejects new container creation with 429 when space container limit is reached", async () => {
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        ...
        if (labels.some((l: string) => l.startsWith("cortexai-agent-hub.botId="))) {
          return [];
        }
        ...
        return [
          {
            ...
            Labels: {
              "cortexai-agent-hub.managed": "true",
              "cortexai-agent-hub.spaceId": "space-1",
            },
          },
          {
            Id: "c2",
  ...
  it("resumes existing container even if space is at limit", async () => {
    ...
    const existing = {
      ...
      inspect: vi.fn().mockResolvedValue({
        ...
        Config: {
          ...
          Labels: {
            "cortexai-agent-hub.managed": "true",
            "cortexai-agent-hub.botId": "bot-existing",
            "cortexai-agent-hub.spaceId": "space-1",
          },
        },
        State: { Running: true },
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        const labels = opts?.filters?.label ?? [];
        if (labels.some((l: string) => l === "cortexai-agent-hub.botId=bot-existing")) {
          return [
            {
              Id: existing.id,
              Labels: {
  ...
  it("counts legacy workspaceId COMPUTER_IMAGE containers toward the limit", async () => {
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        ...
        // Legacy managed computer: COMPUTER_IMAGE + workspaceId, no cortexai-agent-hub.managed.
        return [
          {
            Id: "legacy",
            Image: COMPUTER_IMAGE,
            Labels: {
              "cortexai-agent-hub.workspaceId": "space-1",
              "cortexai-agent-hub.botId": "legacy-bot",
            },
          },
        ];
      },
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        ...
        return Array.from({ length: created }, (_, index) => ({
          Id: `c${index}`,
          Labels: { "cortexai-agent-hub.managed": "true", "cortexai-agent-hub.spaceId": "space-1" },
        }));
      },
    );
