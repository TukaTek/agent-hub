...
function fixture() {
  const root = mkdtempSync(path.join(tmpdir(), "cortexai-agent-hub-profile-test-"));
  fixtures.push(root);
  ...
  mkdirSync(runtime);
  const profile = (bot: string) =>
    browserProfilePathForScreen(bot).replace("/home/cortexai-agent-hub", home);
  const run = (script: string) => {
    ...
    return spawnSync(
      ...
      [
        ...
        "-c",
        portable
          .replaceAll("/tmp/cortexai-agent-hub", runtime)
          .replaceAll("/home/cortexai-agent-hub", home),
      ],
      {
        encoding: "utf8",
        timeout: 10_000,
