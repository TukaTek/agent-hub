...
describe("graphical computer spec", () => {
  it("creates a VNC desktop, not an alpine sleep fallback", () => {
    const options = containerCreateOptions({
      ...
      networkMode: "cortexai-agent-hub_default",
    });
    ...
    expect(options.HostConfig.NetworkMode).toBe("cortexai-agent-hub_default");
  });
  ...
  it("uses the container IP only for the internal screen network topology", () => {
    const networkMode = "cortexai-agent-hub_default";
    expect(
      resolveScreenPublishTarget({
