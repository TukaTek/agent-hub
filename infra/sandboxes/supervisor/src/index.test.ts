  ...
  it("rejects a provision request whose identity headers do not match its body", async () => {
    const response = await supervisorApp.request("/computers", {
      ...
      headers: {
        authorization: `Bearer ${token}`,
        "content-type": "application/json",
        "x-cortexai-agent-hub-bot-id": "other-bot",
...
describe("sandbox supervisor input containment", () => {
  ...
  it("accepts the legacy workspace label without weakening container identity", () => {
    expect(
      hasComputerIdentity(
        { "cortexai-agent-hub.botId": "bot", "cortexai-agent-hub.workspaceId": "space" },
        "bot",
        "space",
      ),
    ).toBe(true);
    expect(
      hasComputerIdentity(
        { "cortexai-agent-hub.botId": "bot", "cortexai-agent-hub.workspaceId": "other-space" },
        "bot",
        "space",
      ),
    ).toBe(false);
      ...
      hasComputerIdentity(
        {
          ...
          "cortexai-agent-hub.workspaceId": "other-space",
        },
        "bot",
        "space",
      ),
  ...
  it("routes Docker browser aliases through the safe wrapper on every display", () => {
    for (const application of DOCKER_BROWSER_ALIASES) {
      expect(
        ...
        argv: ["env", "DISPLAY=:2", "cortexai-agent-hub-browser", "https://example.com"],
      });
    }
  ...
  it("keeps the viewer read-only and uses a separate process for takeover control", () => {
    ...
    expect(interactiveScreenCommand(false, "lease-old")).toContain(
      "CORTEXAI_AGENT_HUB_CONTROL_RELEASED",
    );
  });
  ...
  it("resets stale managed screens without killing unrelated container jobs", () => {
    ...
    expect(command).toContain("for marker in /tmp/cortexai-agent-hub/browser-profile-*");
    expect(command).toContain("/tmp/cortexai-agent-hub/browser-pid-*");
  ...
  it("stops the released bot's browser without tearing down the primary desktop", () => {
    ...
    expect(primary).toContain("rm -f /tmp/cortexai-agent-hub/control-token-1");
