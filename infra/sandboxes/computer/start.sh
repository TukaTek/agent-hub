...
HOME="$AGENT_HOME" cortexai-agent-hub-browser >/tmp/cortexai-agent-hub/browser.log 2>&1 &
browser_up=0
...
if [[ "$browser_up" -ne 1 ]]; then
  ...
  cat /tmp/cortexai-agent-hub/browser.log >&2 || true
  xterm -geometry 100x28+48+48 -bg "#111113" -fg "#E8E8EA" -cr "#E8E8EA" -title "Terminal" >/tmp/cortexai-agent-hub/xterm.log 2>&1 &
fi
...
websockify --heartbeat=30 --web="$NOVNC_ROOT" 0.0.0.0:6080 127.0.0.1:5900 >/tmp/cortexai-agent-hub/novnc.log 2>&1 &
