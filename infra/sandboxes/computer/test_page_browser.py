...
loader = importlib.machinery.SourceFileLoader("page_browser", str(Path(__file__).with_name("cortexai-agent-hub-page-browser")))
...
class PageBrowserTest(unittest.TestCase):
    def test_closed_stdin_cancels_helper_without_a_browser(self):
        with subprocess.Popen([sys.executable, str(Path(__file__).with_name("cortexai-agent-hub-page-browser")), "snapshot", "{}"],
                              env={**os.environ, "CORTEXAI_AGENT_HUB_BROWSER_WATCH_STDIN": "1", "CORTEXAI_AGENT_HUB_CDP_PORT": "0"},
