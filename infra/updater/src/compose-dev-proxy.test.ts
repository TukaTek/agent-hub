...
describe("source-checkout compose postgres hardening", () => {
  it("keeps postgres network-internal and requires POSTGRES_PASSWORD from .env", () => {
    ...
    expect(postgres?.environment?.POSTGRES_USER).toBe("${POSTGRES_USER:-cortexai-agent-hub}");
    expect(postgres?.environment?.POSTGRES_DB).toBe("${POSTGRES_DB:-cortexai-agent-hub}");
    for (const name of ["api", "worker"] as const) {
      expect(compose.services[name]?.environment?.DATABASE_URL).toBe(
        "postgres://${POSTGRES_USER:-cortexai-agent-hub}:${POSTGRES_PASSWORD:?Set POSTGRES_PASSWORD in .env}@postgres:5432/${POSTGRES_DB:-cortexai-agent-hub}",
      );
    }
  });
