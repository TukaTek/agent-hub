...
export function resolveUpdaterConfig(env: NodeJS.ProcessEnv): UpdaterConfig {
  ...
  const composeFiles = (env.CORTEXAI_AGENT_HUB_COMPOSE_FILE?.trim() || DEFAULT_COMPOSE_FILE)
    .split(separator)
    .map((entry) => entry.trim())
    .filter((entry) => entry !== "");
  if (composeFiles.length === 0) {
    throw new Error("CORTEXAI_AGENT_HUB_COMPOSE_FILE must name at least one Compose file.");
  }
  for (const composeFile of composeFiles) {
    if (path.posix.isAbsolute(composeFile) || composeFile.split("/").includes("..")) {
      throw new Error(
        "CORTEXAI_AGENT_HUB_COMPOSE_FILE must list paths inside the deployment directory.",
      );
    }
  }
  ...
  const extraServices = (env.CORTEXAI_AGENT_HUB_UPDATE_SERVICES ?? "")
    .split(",")
    .map((entry) => entry.trim())
    .filter((entry) => entry !== "");
  for (const service of extraServices) {
    if (!SERVICE_NAME.test(service)) {
      throw new Error(
        `CORTEXAI_AGENT_HUB_UPDATE_SERVICES contains an unusable service name: ${service}`,
      );
    }
    ...
    if (service === UPDATER_SERVICE) {
      throw new Error(
        "CORTEXAI_AGENT_HUB_UPDATE_SERVICES must not contain the updater: it cannot recreate itself mid-update.",
      );
    }
  }
