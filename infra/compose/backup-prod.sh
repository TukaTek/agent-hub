...
PROJECT_DIR="${CORTEXAI_AGENT_HUB_DEPLOY_DIR:-/srv/cortexai-agent-hub}"
[[ "${PROJECT_DIR}" == /* ]] || { echo "CORTEXAI_AGENT_HUB_DEPLOY_DIR must be an absolute path" >&2; exit 1; }
...
"${compose[@]}" exec -T postgres sh -c \
  ...
  > "${SNAPSHOT_DIR}/cortexai-agent-hub.dump"
...
"${compose[@]}" exec -T postgres pg_restore --list \
  < "${SNAPSHOT_DIR}/cortexai-agent-hub.dump" >/dev/null
tar -tzf "${SNAPSHOT_DIR}/appdata.tgz" >/dev/null

sha256sum "${SNAPSHOT_DIR}/cortexai-agent-hub.dump" "${SNAPSHOT_DIR}/appdata.tgz" \
...
echo "Verified CortexAI Agent Hub backup written to ${SNAPSHOT_DIR}"
