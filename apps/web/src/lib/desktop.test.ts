...
import {
  type CortexAiAgentHubDesktop,
  type CortexAiAgentHubDesktopOAuthCallback,
  desktopOAuthCode,
  oauthStateOf,
...
function desktop(platform: string): CortexAiAgentHubDesktop {
  const updateState = {
...
describe("attempt correlation", () => {
  ...
  function bridgeEmitting() {
    let emit: (callback: CortexAiAgentHubDesktopOAuthCallback) => void = () => undefined;
    const unsubscribe = vi.fn();
    vi.stubGlobal("window", {
      cortexAiAgentHubDesktop: {
        ...desktop("linux"),
        oauth: {
          onCallback: (listener: (callback: CortexAiAgentHubDesktopOAuthCallback) => void) => {
            emit = listener;
            return unsubscribe;
          },
        },
    ...
    return { emit: (c: CortexAiAgentHubDesktopOAuthCallback) => emit(c), unsubscribe };
  }
