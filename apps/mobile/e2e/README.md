...
expects a running CortexAI Agent Hub stack and deliberately stays out of ordinary pull-request CI.
...
4. Build/install the native app with an API URL that the emulator can reach. For the standard local
   ...
   ```sh
   EXPO_PUBLIC_API_URL=http://10.0.2.2:3100 pnpm --filter @cortexai-agent-hub/mobile android
   ```
...
```sh
pnpm --filter @cortexai-agent-hub/mobile test:e2e -- \
  -e CORTEXAI_AGENT_HUB_E2E_EMAIL=mobile-smoke@example.test \
  -e CORTEXAI_AGENT_HUB_E2E_PASSWORD='replace-with-the-disposable-password' \
  -e CORTEXAI_AGENT_HUB_E2E_BOT_NAME=MaestroSmoke-001 \
  -e CORTEXAI_AGENT_HUB_E2E_MESSAGE=mobile-smoke-message
```
