...
describe("desktop stack token transport", () => {
  it("allows HTTPS and loopback HTTP only", () => {
    expect(maySendDesktopStackToken("https://cortexai-agent-hub.example.com")).toBe(true);
    ...
    expect(maySendDesktopStackToken("http://cortexai-agent-hub.local:5173")).toBe(false);
  });
});
