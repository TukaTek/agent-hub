...
describe("stack identity", () => {
  let dir: string;
  beforeEach(async () => {
    dir = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-stack-token-"));
  });
  afterEach(async () => {
    await rm(dir, { recursive: true, force: true });
  });
...
describe("LocalStackController", () => {
  ...
  it("installs the compose project and walks every phase to ready", async () => {
    ...
    expect(calls.at(-1)?.env.CORTEXAI_AGENT_HUB_DESKTOP_STACK_TOKEN).toBe("ab".repeat(32));
  ...
  it("keeps lifecycle commands off the standalone project despite environment overrides", async () => {
    ...
    await writeFile(path.join(dir, STACK_ENV_FILE), "COMPOSE_PROJECT_NAME=cortexai-agent-hub\n");
    const stack = controller({ env: { COMPOSE_PROJECT_NAME: "cortexai-agent-hub" } }, (args) =>
      args[7] === "up" ? { code: 1, stderr: "port is already allocated" } : ok(args),
    );
    expect((await stack.start()).phase).toBe("failed");
    // A fresh process must stop the same desktop project without discovering or adopting cortexai-agent-hub.
    ...
    for (const call of commands) {
      expect(call.args.slice(5, 7)).toEqual(["--project-name", "cortexai-agent-hub-desktop"]);
  ...
  it("retries port conflicts on a new origin and uses it for auth, health, and saved launches", async () => {
    ...
    expect(
      calls
        .filter((call) => call.args[7] === "up")
        .map((call) => call.env.CORTEXAI_AGENT_HUB_WEB_PORT),
    ).toEqual(["5173", "45174"]);
    expect(calls.at(-1)?.env).toMatchObject({
      CORTEXAI_AGENT_HUB_API_PORT: "0",
      ...
      const stack = controller({}, (args) => {
        if (args[7] === "up" && exhausted) {
          return {
            code: 1,
            [stream]:
              "failed to create network cortexai-agent-hub-desktop_data: Error response from daemon: all predefined address pools have been fully subnetted",
          };
        }
        return ok(args);
      });
