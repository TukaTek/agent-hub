...
beforeEach(async () => {
  userData = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-setup-"));
});
...
describe("setup store", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      await writeFile(
        target,
        JSON.stringify({ mode: "existing", serverUrl: "https://cortexai-agent-hub.example.com" }),
        "utf8",
      );
