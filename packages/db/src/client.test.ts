...
describe("createDb", () => {
  it("caps the pg pool and swallows idle-client errors so they cannot crash the process", () => {
    const { pool } = createDb(
      "postgres://cortexai-agent-hub:cortexai-agent-hub@127.0.0.1:9/cortexai-agent-hub",
      {
        poolMax: 3,
        applicationName: "cortexai-agent-hub-test",
      },
    );
    pools.push(pool);
    ...
    expect(pool.options.application_name).toBe("cortexai-agent-hub-test");
...
describe("createPool", () => {
  it("builds a bounded pool with connect-retry and idle-error listeners", () => {
    const pool = createPool(
      ...
      {
        poolMax: 2,
        applicationName: "cortexai-agent-hub-pool-test",
      },
    );
    pools.push(pool);
    ...
    expect(pool.options.connectionTimeoutMillis).toBe(10_000);
    expect(pool.options.idleTimeoutMillis).toBe(0);
    expect(pool.options.application_name).toBe("cortexai-agent-hub-pool-test");
    expect(pool.listenerCount("error")).toBeGreaterThan(0);
    expect(pool.listenerCount("connect")).toBeGreaterThan(0);
    expect(() => pool.emit("error", new Error("idle client lost"))).not.toThrow();
  });
