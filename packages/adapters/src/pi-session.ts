 ...
 * This is deliberately a recorder only: CortexAI Agent Hub remains responsible for running
...
export class PiJsonlSessionRecorder implements PiSessionRecorder {
  ...
  async start(input: PiSessionStart): Promise<PiSessionHandle> {
    ...
    await handle.appendCustomEntry("cortexai-agent-hub_context", {
      cortexAiAgentHubUserId: input.userId,
      cortexAiAgentHubBotId: input.botId,
      cortexAiAgentHubRunId: input.runId,
      cortexAiAgentHubThreadId: input.threadId,
      ...(input.traceId ? { cortexAiAgentHubTraceId: input.traceId } : {}),
      model: input.model,
