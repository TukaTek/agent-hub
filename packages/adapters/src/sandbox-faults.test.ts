...
import type { PortableFile, SandboxProvider } from "@cortexai-agent-hub/adapter-kit";
...
describe.each([
  ...
  [
    "desktop",
    async () => {
      const root = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-sandbox-faults-"));
...
async function providerSet(label: string): Promise<Array<[string, SandboxProvider]>> {
  const root = await mkdtemp(path.join(tmpdir(), `cortexai-agent-hub-sandbox-transfer-${label}-`));
  temporaryRoots.push(root);
  return [
