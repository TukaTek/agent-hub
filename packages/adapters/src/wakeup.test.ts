import type { BackgroundJobHandlers } from "@cortexai-agent-hub/adapter-kit";
...
describe("InMemoryJobQueue", () => {
  ...
  it("keeps wrapped payloads readable by workers that do not unwrap envelopes", async () => {
    const { wrapJobPayload } = await import("@cortexai-agent-hub/logging");
    const { parseBackgroundJob } = await import("@cortexai-agent-hub/adapter-kit");
