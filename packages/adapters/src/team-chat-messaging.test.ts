import type { MessagingInboundMessage, MessagingSurface } from "@cortexai-agent-hub/adapter-kit";
...
describe("toTeamChatInbound", () => {
  ...
  it("treats @mentions as mention when kind is omitted", () => {
    expect(toTeamChatInbound(baseEvent({ content: "hey @cortexai-agent-hub look" }))?.kind).toBe(
      "mention",
    );
  });
});
