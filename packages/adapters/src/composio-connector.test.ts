...
describe("composio tool mapping", () => {
  ...
  it("retains account pins from subsequent active-account pages", async () => {
    composioSdkState.created.length = 0;
    composioSdkState.sessions.clear();
    composioToolkitDirectory.invalidate();
    const previous = composioSdkState.connectedAccounts.list;
    const list = vi.fn(async (query?: Record<string, unknown>) =>
      query?.cursor === "page-2"
        ? { items: [{ id: "ca-work" }] }
        : { items: [{ id: "ca-personal" }], nextCursor: "page-2" },
    );
    composioSdkState.connectedAccounts.list = list;
    try {
      await new ComposioConnector().discoverTools({
        operationId: "paged-accounts",
        traceId: "paged-accounts",
        spaceId: "workspace",
        userId: "user-1",
        signal: new AbortController().signal,
        connectedConnections: ["ca-personal", "ca-work"].map((providerRef) => ({
          id: providerRef,
          connectorId: "composio",
          externalId: "github",
          displayName: "GitHub",
      ...
      expect(composioSdkState.created.at(-1)?.config.connectedAccounts).toEqual({
        GITHUB: ["ca-personal", "ca-work"],
      });
      expect(composioSdkState.created.at(-1)?.config.multiAccount).toEqual(
  ...
  it("uses catalog-canonical toolkit slugs without preloading every tool", async () => {
    ...
    expect(
      ...
      { userId: "__cortexai-agent-hub_catalog__", toolkits: undefined, sessionPreset: undefined },
