...
export function desktopCommandResponder() {
  ...
  return (command: string): { exitCode: number; stdout: string; stderr: string } | undefined => {
    ...
    if (key && command.includes("CORTEXAI_AGENT_HUB_DESKTOP=")) {
      ...
      return {
        exitCode: 0,
        stdout: `CORTEXAI_AGENT_HUB_DESKTOP=${index}:view-${key}\n`,
        stderr: "",
      };
    }
    if (key && command.includes("CORTEXAI_AGENT_HUB_DESKTOP_RELEASED=")) {
      slots.delete(key);
