...
describe("computer provisioning", () => {
  ...
  it("reclaims a stale suspending computer after the wait times out", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-stale-suspend-reclaim-"));
  ...
  it("does not adopt a concurrent reclaim stamp after claiming", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-claim-stamp-race-"));
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
      providerRef: null as string | null,
  ...
  it("advances the claim stamp past the observed updatedAt when the clock does not move", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-claim-stamp-skew-"));
    const observed = new Date("2024-01-01T00:00:00.000Z");
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
  ...
  it("does not reclaim a fresh booting claim without waiting", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-fresh-boot-claim-"));
    const nowMs = Date.parse("2024-06-01T12:00:00.000Z");
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
  ...
  it("does not reclaim a stale booting claim while another run still holds a live worker lease", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-stale-boot-live-run-"));
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
      providerRef: null as string | null,
  ...
  it("activates a boot even if another Team bot takes a lease mid-provision", async () => {
    const dataDir = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-team-lease-mid-boot-"));
    const row = {
      id: "computer-1",
      homeKey: "bot-1",
      providerRef: null as string | null,
