...
describe("PostgresRealtimeFanout", () => {
  ...
  it("shares one fixed-channel listener across many logical subscribers", async () => {
    ...
    await vi.waitFor(() =>
      expect(clients[0]?.queries).toEqual(["LISTEN cortexai_agent_hub_events"]),
    );
    received.forEach((subscriber) => {
      subscriber.mockClear();
    });
    clients[0]?.notify(
      "cortexai_agent_hub_events",
    ...
    clients[0]?.notify("cortexai_agent_hub_events", "malformed");
  ...
  it("publishes a small envelope and rejects oversized signals", async () => {
    ...
    expect(published[0]?.values[0]).toBe("cortexai_agent_hub_events");
