...
describe("resolveAutoReviewChecker", () => {
  it("prefers explicit env overrides", () => {
    expect(
      resolveAutoReviewChecker({
        CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER: "openrouter",
        CORTEXAI_AGENT_HUB_AUTO_REVIEW_MODEL: "cheap/fast",
        CORTEXAI_AGENT_HUB_LOCAL_MODELS: "local-a",
  ...
  it("prefers local models when configured", () => {
    expect(
      resolveAutoReviewChecker({
        CORTEXAI_AGENT_HUB_LOCAL_MODELS: " llama-local , other ",
  ...
  it("selects Jev when the provider is jev and a TypeSafe key is present", () => {
    expect(
      resolveAutoReviewChecker({
        CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER: "jev",
    ...
    expect(
      resolveAutoReviewChecker({
        CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER: "jev",
        CORTEXAI_AGENT_HUB_AUTO_REVIEW_MODEL: "jev-custom",
        TYPESAFE_API_KEY: "ts-key",
      }),
...
describe("resolveAutoReviewProviderKind", () => {
  it("uses Jev only when a TypeSafe key is present", () => {
    expect(resolveAutoReviewProviderKind({ CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER: "jev" })).toBe(
      "llm",
    );
    expect(
      resolveAutoReviewProviderKind({
    ...
    expect(
      resolveAutoReviewProviderKind({ CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER: "openrouter" }),
    ).toBe("llm");
    expect(
      resolveAutoReviewProviderKind({
        CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER: "scripted",
        AGENT_RUNTIME: "scripted",
      }),
    ).toBe("scripted");
    expect(
      resolveAutoReviewProviderKind({ CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER: "scripted" }),
    ).toBe("llm");
  });
});
...
describe("isAutoReviewCheckerConfigured", () => {
  it("requires a runnable checker", () => {
    ...
    expect(
      isAutoReviewCheckerConfigured({
        env: { CORTEXAI_AGENT_HUB_LOCAL_MODELS: "local-1" },
      }),
    ).toBe(true);
    expect(
      isAutoReviewCheckerConfigured({
    ...
    expect(
      isAutoReviewCheckerConfigured({
        env: { CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER: "jev", TYPESAFE_API_KEY: "ts-key" },
      }),
    ).toBe(true);
    expect(
      isAutoReviewCheckerConfigured({
    ...
    expect(
      isAutoReviewCheckerConfigured({
        env: { CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER: "scripted", AGENT_RUNTIME: "scripted" },
      }),
    ).toBe(true);
    expect(
      isAutoReviewCheckerConfigured({
        env: { CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER: "scripted" },
      }),
    ).toBe(false);
  });
});
...
describe("deploymentAutoReviewDefault", () => {
  it("defaults off", () => {
    ...
    expect(deploymentAutoReviewDefault({ CORTEXAI_AGENT_HUB_AUTO_REVIEW: "1" })).toBe(true);
    expect(deploymentAutoReviewDefault({ CORTEXAI_AGENT_HUB_AUTO_REVIEW: "true" })).toBe(true);
  });
});
...
describe("autoReviewTimeoutMs", () => {
  it("defaults to 1500 and clamps bad values", () => {
    ...
    expect(autoReviewTimeoutMs({ CORTEXAI_AGENT_HUB_AUTO_REVIEW_TIMEOUT_MS: "2000" })).toBe(2_000);
    expect(autoReviewTimeoutMs({ CORTEXAI_AGENT_HUB_AUTO_REVIEW_TIMEOUT_MS: "nope" })).toBe(1_500);
  });
});
...
describe("autoReviewMinConfidence", () => {
  it("defaults to 0.5 and clamps out of range", () => {
    ...
    expect(autoReviewMinConfidence({ CORTEXAI_AGENT_HUB_AUTO_REVIEW_MIN_CONFIDENCE: "0.8" })).toBe(
      0.8,
    );
    expect(autoReviewMinConfidence({ CORTEXAI_AGENT_HUB_AUTO_REVIEW_MIN_CONFIDENCE: "2" })).toBe(
      0.5,
    );
    expect(autoReviewMinConfidence({ CORTEXAI_AGENT_HUB_AUTO_REVIEW_MIN_CONFIDENCE: "nope" })).toBe(
      0.5,
    );
  });
});
