...
import type { AutoReviewJudgeDecision } from "@cortexai-agent-hub/core";
import { redactSecrets } from "@cortexai-agent-hub/core";
...
export function autoReviewMinConfidence(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.CORTEXAI_AGENT_HUB_AUTO_REVIEW_MIN_CONFIDENCE?.trim();
...
/**
 * `CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER=jev` selects TypeSafe Jev when a key is present;
...
export function resolveAutoReviewProviderKind(
  ...
  const requested = env.CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER?.trim().toLowerCase();
...
export function resolveAutoReviewChecker(
  ...
  if (kind === "jev") {
    return {
      ...
      model: env.CORTEXAI_AGENT_HUB_AUTO_REVIEW_MODEL?.trim() || DEFAULT_JEV_MODEL,
    };
  }
  if (kind === "scripted") {
