...
/**
 * Construct the Auto Review adapter selected by `CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER`.
  ...
  switch (kind ?? resolveAutoReviewProviderKind(env)) {
    case "jev": {
      ...
      return new JevAutoReviewProvider({
        ...
        model:
          options.model ?? env.CORTEXAI_AGENT_HUB_AUTO_REVIEW_MODEL?.trim() ?? DEFAULT_JEV_MODEL,
