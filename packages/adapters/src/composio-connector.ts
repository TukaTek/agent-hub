  ...
  private async listActiveAccountIds(
    ...
    try {
      ...
      for (let page = 0; page < 200; page += 1) {
        ...
        if (!listed.nextCursor) return ids;
        if (seenCursors.has(listed.nextCursor)) return null;
        seenCursors.add(listed.nextCursor);
        cursor = listed.nextCursor;
      }
      // Only a complete lookup is an allowlist; incomplete results must not strip pins.
      return null;
    } catch {
      return null;
    }
