...
describe("Pi JSONL sessions", () => {
  ...
  it("uses Pi's session format for context and completed messages", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "cortexai-agent-hub-pi-session-"));
    try {
      ...
      expect(records).toContainEqual(
        expect.objectContaining({
          kind: "entry",
          customType: "cortexai-agent-hub_context",
          data: expect.objectContaining({
            cortexAiAgentHubThreadId: "thread-1",
            cortexAiAgentHubTraceId: "trace-1",
          }),
        }),
      );
  ...
  it("keeps sessions scoped and bounded by the retention policy", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "cortexai-agent-hub-pi-retention-"));
    try {
  ...
  it("removes bot and account session scopes", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "cortexai-agent-hub-pi-cleanup-"));
    try {
