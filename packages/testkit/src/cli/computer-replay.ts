...
async function main() {
  ...
  if (unknown.length)
    throw new Error(
      "Usage: computer-replay.ts [--image=cortexai-agent-hub/computer:local] [--subnet=unused-private-CIDR] [--live --record=new-fixture.json]",
    );
  const image = imageArg?.slice("--image=".length) || "cortexai-agent-hub/computer:local";
  ...
  const directory = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-computer-replay-"));
  ...
  Object.assign(env, {
    ...
    CORTEXAI_AGENT_HUB_COMPUTER_IMAGE: image,
