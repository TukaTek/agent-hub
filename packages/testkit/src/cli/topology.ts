...
import type { RunsListOutput, ThreadSnapshot } from "@cortexai-agent-hub/contracts";
import { isTerminal } from "@cortexai-agent-hub/core";
...
const project = `cortexai-agent-hub-topology-${process.pid}-${Date.now().toString(36)}`;
...
async function main() {
  ...
  const env = {
    ...
    TOPOLOGY_APP_IMAGE: process.env.TOPOLOGY_APP_IMAGE ?? "cortexai-agent-hub/app:topology",
    TOPOLOGY_COMPUTER_IMAGE:
      process.env.TOPOLOGY_COMPUTER_IMAGE ?? "cortexai-agent-hub/computer:topology",
  };
...
async function waitForWorker(
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    // {"timestamp":...,"level":"info","message":"worker ready","service.name":"cortexai-agent-hub-worker"},
    // not the old plain "cortexai-agent-hub worker ready" console.log line -- match the JSON field directly
...
function assertComputerNetworkIsolation(
  ...
  const managed = docker([
    ...
    computerId,
    "--format",
    '{{index .Config.Labels "cortexai-agent-hub.managed"}}',
  ]).trim();
  ...
  try {
    ...
    for (const networkName of networks) {
      const ids = docker([
        ...
        `network=${networkName}`,
        "--filter",
        "label=cortexai-agent-hub.managed=true",
      ])
        .split("\n")
        .map((id) => id.trim())
        .filter(Boolean);
