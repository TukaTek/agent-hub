...
import type { ComputerRef, SandboxProvider } from "@cortexai-agent-hub/adapter-kit";
...
describeLive("real model and E2B computer journey", () => {
  ...
  beforeAll(async () => {
    ...
    dataDir = mkdtempSync(path.join(tmpdir(), "cortexai-agent-hub-computer-e2e-"));
...
PAGE = b'''<!doctype html><meta charset="utf-8"><title>CortexAI Agent Hub visual test</title>
