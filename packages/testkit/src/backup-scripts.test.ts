...
function fixture() {
  const root = mkdtempSync(path.join(os.tmpdir(), "cortexai-agent-hub-backup-"));
...
describe("development backup failures", () => {
  ...
  it.each(["absent", "skipped"])("allows an empty archive when data is %s", (scenario) => {
    ...
    const result = f.run("scripts/backup.sh", ["example"], {
      CORTEXAI_AGENT_HUB_BACKUP_SKIP_HOMES: scenario === "skipped" ? "1" : "0",
    });
...
describe("production backup deployment and archive behavior", () => {
  it.each([false, true])("uses the configured deployment directory: %s", (custom) => {
    const f = fixture();
    const deployment = custom ? f.checkout : "/srv/cortexai-agent-hub";
    const result = f.run(
      "infra/compose/backup-prod.sh",
      [],
      custom ? { CORTEXAI_AGENT_HUB_DEPLOY_DIR: deployment } : {},
    );
    expect(result.status, result.stderr).toBe(0);
    expect(result.stdout).toContain("Verified CortexAI Agent Hub backup");
  ...
  it("provides optional systemd configuration for installed copies", () => {
    const service = readFileSync(
      path.join(repoRoot, "infra/systemd/cortexai-agent-hub-backup.service"),
      "utf8",
    );
    expect(service).toContain("EnvironmentFile=-/etc/cortexai-agent-hub/backup.env");
  });
  ...
  it.each([{ FAIL_TAR: "2" }, { FAIL_DUMP: "1" }, { FAIL_VERIFY: "1" }])(
    ...
    (failure) => {
      ...
      expect(result.status).not.toBe(0);
      expect(result.stdout).not.toContain("Verified CortexAI Agent Hub backup");
    },
  );
});
