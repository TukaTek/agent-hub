...
describe.skipIf(!databaseAvailable)("offline Pi computer approval", () => {
  beforeAll(() => {
    ...
    vi.stubEnv("CORTEXAI_AGENT_HUB_OPENAI_COMPATIBLE_VISION_MODELS", "offline-fixture");
  });
  ...
  it.each(["allow", "deny"] as const)(
    ...
    async (answer) => {
      ...
      const dataDir = await mkdtemp(path.join(tmpdir(), "cortexai-agent-hub-computer-approval-"));
