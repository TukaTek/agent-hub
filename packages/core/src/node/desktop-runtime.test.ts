    ...
    spawnSync(
      ...
      [
        ...
        [
          ...
          script
            .replaceAll("/tmp/cortexai-agent-hub/desktop-assignments", root)
            .replaceAll("/tmp/cortexai-agent-hub", root),
        ].join("\n"),
      ],
...
describe("shared Linux desktop lifecycle", () => {
  it("allocates live slots past 1000 bots, keeps assignments across callers, and rejects stale leases", () => {
    const f = fixture();
    expect(f.ensure("a").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=0:view-a");
    expect(f.ensure("b").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=1:view-b");
    expect(f.ensure("a", "new:2").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=0:view-a");
    ...
    expect(f.ensure("bot-1000").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=1000:view-bot-1000");
    expect(f.release("a", "new:2").status).toBe(0);
    expect(f.ensure("c").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=0:view-c");
    expect(f.ensure("b").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=1:view-b");
  });
  ...
  it("reserves failed startup and teardown slots until a successful retry", () => {
    ...
    expect(f.ensure("c").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=2:view-c");
    expect(f.release("a").status).toBe(0);
    expect(f.ensure("d").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=0:view-d");
  });
  ...
  it("skips damaged assignments while reserving their live display markers", () => {
    ...
    expect(f.ensure("a").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=1:view-a");
    expect(f.ensure("b").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=2:view-b");
    expect(f.ensure("a").stdout).toContain("CORTEXAI_AGENT_HUB_DESKTOP=1:view-a");
  });
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          "pkill() { :; }; sleep() { :; }",
          `bash() { printf '%s\\n' "$5" >>${shellQuote(record)}; }`,
          resetDesktopRuntimeCommand(env).replaceAll("/tmp/cortexai-agent-hub", f.root),
        ].join("\n"),
      ],
      { encoding: "utf8", timeout: 5000 },
    );
  ...
  it("stops only the selected VNC transport and keeps the shared gateway", () => {
    ...
    expect(
      patterns.some((pattern) =>
        pattern.test(
          "/usr/bin/x11vnc -display :21 -rfbport 0 -unixsock /tmp/cortexai-agent-hub/sockets/view-21-token -forever",
        ),
      ),
    ).toBe(true);
  });
