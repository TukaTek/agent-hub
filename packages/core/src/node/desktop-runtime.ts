...
function browserPidPathForScreen(screenId: string) {
  return `/tmp/cortexai-agent-hub/browser-pid-${browserKeyForScreen(screenId)}`;
}
...
export function browserLauncherPath(displayNumber: number | string) {
  return `/tmp/cortexai-agent-hub/browser-launch-${displayNumber}`;
}
...
export function stopAllDesktopBrowsersCommand(env = DEFAULT_DESKTOP_ENV) {
  const placeholder = "CORTEXAI_AGENT_HUB_INTERNAL_PROFILE";
  ...
  return [
    ...
    '  pid_file="/tmp/cortexai-agent-hub/browser-pid-$hash"',
...
export function resetDesktopRuntimeCommand(env = DEFAULT_DESKTOP_ENV) {
  ...
  return [
    ...
    "for marker in /tmp/cortexai-agent-hub/browser-profile-*; do",
    '  [ -f "$marker" ] || continue',
    // biome-ignore lint/suspicious/noTemplateCurlyInString: generated shell parameter expansion
    "  display=${marker##*-}",
    ...
    `rm -f ${TARGETS}/view-* ${TARGETS}/control-* /tmp/cortexai-agent-hub/browser-pid-* /tmp/cortexai-agent-hub/browser-profile-* /tmp/cortexai-agent-hub/browser-launch-*`,
  ].join("\n");
}
...
function revokeTargetCommand(kind: "view" | "control", display: number | string) {
  return `mkdir -p ${TARGETS}; : >/tmp/cortexai-agent-hub/${kind}-target-next-${display}; mv /tmp/cortexai-agent-hub/${kind}-target-next-${display} ${TARGETS}/${kind}-${display}`;
}

function stopVncCommand(kind: "view" | "control", layout: ReturnType<typeof commandLayout>) {
  const socketPrefix = `/tmp/cortexai-agent-hub/sockets/${kind}-${layout.displayNumber}-`;
...
function gatewayCommand(port: string) {
  ...
  return [
    ...
    `exec 10>/tmp/cortexai-agent-hub/gateway-${port}.lock; flock -w 120 10`,
    ...
    `  nohup "$proxy" --heartbeat=30 --web="$web" --token-plugin=TokenFile --token-source=${TARGETS} 0.0.0.0:${port} 8>&- 9>&- 10>&- </dev/null >/tmp/cortexai-agent-hub/gateway-${port}.log 2>&1 &`,
    "fi",
...
function renderStopExtraScreenCommand(
  ...
  return [
    ...
    `rm -f /tmp/cortexai-agent-hub/browser-profile-${layout.displayNumber} ${browserLauncherPath(layout.displayNumber)}`,
...
function renderEnsureScreenCommand(
  ...
  const setupDisplay =
    index === 0 && env.preservePrimaryDisplay
      ...
      : [
          ...
          `  mkdir -p /tmp/cortexai-agent-hub ${fluxHome}/.fluxbox /tmp/.X11-unix`,
          `  rm -f /tmp/.X${layout.displayNumber}-lock /tmp/.X11-unix/X${layout.displayNumber}`,
  ...
  const socket = `/tmp/cortexai-agent-hub/sockets/view-${layout.displayNumber}-\${desktop_view_token}`;
  const setupView = [
    `mkdir -p ${TARGETS} /tmp/cortexai-agent-hub/sockets`,
  ...
  return [
    ...
    ...setupDisplay,
    `xdpyinfo -display ${layout.display} >/dev/null 2>&1 || exit 1`,
    `mkdir -p /tmp/cortexai-agent-hub ${shellQuote(path.posix.dirname(profile))}`,
    ...browserRunningFunction(profile, pidFile),
    `printf %s ${shellQuote(profile)} >/tmp/cortexai-agent-hub/browser-profile-${layout.displayNumber}`,
    "if ! browser_running; then",
...
export function interactiveScreenCommand(
  ...
  if (!interactive) {
    ...
    return [
      ...
      stopProcesses,
      "printf 'CORTEXAI_AGENT_HUB_CONTROL_RELEASED\\n'",
      "fi",
    ].join("\n");
  }
  if (!controlToken) throw new Error("interactive screen requires a control token");
  const socket = `/tmp/cortexai-agent-hub/sockets/control-${layout.displayNumber}-${browserKeyForScreen(controlToken)}`;
  return [
    ...
    `nohup x11vnc -display ${layout.display} -forever -shared -nopw -rfbport 0 -unixsock ${socket} -xkb -ncache 0 8>&- 9>&- </dev/null >/tmp/cortexai-agent-hub/x11vnc-control-${layout.displayNumber}.log 2>&1 &`,
    `for i in $(seq 1 50); do [ -S ${socket} ] && break; sleep 0.1; done`,
    `[ -S ${socket} ] || exit 1`,
    `printf '%s: unix_socket:%s\\n' ${shellQuote(controlToken)} ${socket} >/tmp/cortexai-agent-hub/control-target-next-${layout.displayNumber}`,
    `mv /tmp/cortexai-agent-hub/control-target-next-${layout.displayNumber} ${targetFile}`,
    gatewayCommand(layout.controlPort),
  ].join("\n");
}

const REGISTRY = "/tmp/cortexai-agent-hub/desktop-assignments";
const TOKEN_PLACEHOLDER = "CORTEXAI_AGENT_HUB_INTERNAL_VIEW_TOKEN";
...
for marker in pathlib.Path("/tmp/cortexai-agent-hub").glob("browser-profile-*"):
...
`)} "$dir")`,
    ...
    'printf "CORTEXAI_AGENT_HUB_DESKTOP=%s:%s\\n" "$index" "$view_token"',
  ].join("\n");
}
...
export function releaseDesktopCommand(
  ...
  return [
    ...
    'rm -f "$slot"',
    'printf "CORTEXAI_AGENT_HUB_DESKTOP_RELEASED=%s\\n" "$index"',
  ].join("\n");
}
