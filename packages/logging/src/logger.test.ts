...
describe("logger", () => {
  it("filters levels including off", () => {
    ...
    const silent = createLogger({ service: "cortexai-agent-hub-api", level: "off", sinks: [sink] });
    silent.error("never");
    expect(sink.events.map((event) => event.message)).toEqual(["w", "e"]);
  });
  ...
  it("merges child bindings under call-site bindings", () => {
    ...
    expect(sink.events[0]).toMatchObject({
      message: "hello",
      "service.name": "cortexai-agent-hub-api",
      "service.role": "api",
      "job.type": "override",
    });
  });
  ...
  it("keeps the configured service name when bindings try to overwrite it", () => {
    const sink = createTestSink();
    const logger = createLogger({ service: "cortexai-agent-hub-api", sinks: [sink] });
    logger.info("svc", { "service.name": "spoofed" });
    expect(sink.events[0]?.["service.name"]).toBe("cortexai-agent-hub-api");
  });
