...
afterEach(() => {
  installLogger(createLogger({ service: "cortexai-agent-hub", level: "off", sinks: [] }));
});
...
describe("job correlation envelope", () => {
  ...
  it("uses a nested envelope for reserved keys and non-plain objects", () => {
    ...
    const reserved = wrapJobPayload({ runId: "run-2", __cortexAiAgentHubLog: "keep" }, correlation);
    expect(reserved).toEqual({
      v: 1,
      correlation,
      payload: { runId: "run-2", __cortexAiAgentHubLog: "keep" },
    });
