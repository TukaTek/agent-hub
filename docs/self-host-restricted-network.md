...
| Cannot fetch Compose files | `CORTEXAI_AGENT_HUB_DOWNLOAD_BASE`, `--local`, or `CORTEXAI_AGENT_HUB_DOWNLOAD_SKIP_EXISTING=1` |
| Cannot pull app or computer images | `CORTEXAI_AGENT_HUB_IMAGE`, `CORTEXAI_AGENT_HUB_IMAGE_TAG`, `CORTEXAI_AGENT_HUB_COMPUTER_IMAGE`, `CORTEXAI_AGENT_HUB_COMPUTER_IMAGE_TAG` |
...
curl -fsSL -o install-images.sh "${CORTEXAI_AGENT_HUB_INSTALLER_URL}" &&
bash install-images.sh --prepare-only
```

`CORTEXAI_AGENT_HUB_INSTALLER_URL` selects the script for this curl command; `CORTEXAI_AGENT_HUB_DOWNLOAD_BASE` selects
...
```env
CORTEXAI_AGENT_HUB_IMAGE=registry.example.com/mirror/cortexai-agent-hub/app
CORTEXAI_AGENT_HUB_IMAGE_TAG=edge
CORTEXAI_AGENT_HUB_COMPUTER_IMAGE=registry.example.com/mirror/cortexai-agent-hub/computer
