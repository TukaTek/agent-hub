...
from first init (often the former hardcoded `cortexai-agent-hub` / `cortexai-agent-hub`). Keep those
...
- `CORTEXAI_AGENT_HUB_UPDATER_TOKEN` (only if the updater profile is enabled; ≥32 chars,
...
| `TYPESAFE_API_KEY` | Optional TypeSafe Jev Auto Review verifier (`CORTEXAI_AGENT_HUB_AUTO_REVIEW_PROVIDER=jev`) |
