...
Each workspace gets one Team Computer by default. Bots share its files and installed tools. Each Team Assistant starts in `bots/<bot-id>/`, while deliberately shared work belongs in `shared/`. These folders organize work but are not security boundaries: every Team Assistant can access the full Team workspace. A Assistant can instead use a Private Computer, where the whole workspace is its home.

Each active Team Assistant gets its own X display and Chrome process, with a persistent Chrome profile keyed to the Assistant's identity. Logins, cookies, and browser history are independent. Profiles are never cloned from another Assistant, merged, or deleted when a desktop is released. Both Assistants keep their changes; reopening a Assistant uses its existing profile even when its display slot changes. Team runs use fenced per-Assistant database leases: different Assistants can operate concurrently, and one Assistant has only one computer driver at a time.
