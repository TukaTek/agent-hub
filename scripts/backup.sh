...
mkdir -p "$OUT"
"${compose[@]}" exec -T postgres pg_dump -U cortexai-agent-hub cortexai-agent-hub > "$OUT/cortexai-agent-hub.sql"
if [[ "${CORTEXAI_AGENT_HUB_BACKUP_SKIP_HOMES:-0}" == "1" ]] || [[ ! -e "$ROOT/data" && ! -L "$ROOT/data" ]]; then
