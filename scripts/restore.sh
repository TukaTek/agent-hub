...
until "${compose[@]}" exec -T postgres pg_isready -U cortexai-agent-hub >/dev/null 2>&1; do
  sleep 1
done
...
"${compose[@]}" exec -T postgres psql -X -U cortexai-agent-hub -d cortexai-agent-hub \
  --set=ON_ERROR_STOP=on --single-transaction --file=- < "$SRC/cortexai-agent-hub.sql"
